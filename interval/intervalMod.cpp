...
interval interval_algebra::Mod(const interval& x, double m) const
{
    ...
    // [0,m[
    return {0.0, nextafter(fabs(m), 0.0)};
}
...
interval interval_algebra::Mod(const interval& x, const interval& y) const
{
    ...
    if (x.lo() < 0) {
        return reunion(Mod({x.lo(), -1.0}, {y.lo(), y.hi()}), Mod({0.0, x.hi()}, {y.lo(), y.hi()}));
    ...
    if (y.lo() <= 0) {
        return Mod({x.lo(), x.hi()}, {1.0, std::max(-y.lo(), y.hi())});
    ...
    if (x.hi() - x.lo() >= y.lo()) {
        return reunion({0.0, x.hi() - x.lo() - 1}, Mod({x.lo(), x.hi()}, {x.hi() - x.lo() + 1, y.hi()}));
...
void interval_algebra::testMod() const
{
    ...
    check("test algebra Mod", Mod(interval(0, 100), 2), interval(0.0, nextafter(2.0, 0)));
    check("test algebra Mod", Mod(interval(0, 100), -1.0), interval(0.0, nextafter(1.0, 0)));
    ...
    check("test algebra Mod", Mod(interval(0, 100), interval(7, 7)), interval(0.0, nextafter(7.0, 0.0)));
}
}  // namespace itv
